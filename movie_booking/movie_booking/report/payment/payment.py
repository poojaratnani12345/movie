...
def execute(filters: dict | None = None):
	...
	total_revenue=sum(row['total_cost'] for row in data if row['booking_status']=="Confirmed")
...
def get_summary(data):
    total_success = sum(row["total_cost"] for row in data if row["booking_status"] == "Confirmed")
    total_failed = sum(row["total_cost"] for row in data if row["booking_status"] == "Pending")
    
    return [
        {"label": "Total Revenue (Confirmed)", "value": total_success, "indicator": "Green"},
